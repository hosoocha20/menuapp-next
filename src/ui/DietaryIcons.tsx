interface Props {
    width?: string;
    height?: string;
    fontSize?: string;
}

export const ChilliIcon = () => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      fill="#c11b17"
      stroke="#c11b17"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <path stroke="none" d="M0 0h24v24H0z" fill="none" />
      <path d="M13 11c0 2.21 -2.239 4 -5 4s-5 -1.79 -5 -4a8 8 0 1 0 16 0a3 3 0 0 0 -6 0" />
      <path d="M16 8c0 -2 2 -4 4 -4" />
    </svg>
  );
};

export const VeganIcon = () => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      className="icon icon-tabler icons-tabler-outline icon-tabler-leaf"
    >
      <path stroke="none" d="M0 0h24v24H0z" fill="none" />
      <path d="M5 21c.5 -4.5 2.5 -8 7 -10" />
      <path d="M9 18c6.218 0 10.5 -3.288 11 -12v-2h-4.014c-9 0 -11.986 4 -12 9c0 1 0 3 2 5h3z" />
    </svg>
  );
};
export const VGIcon = (props: Props) => {
  return (
<svg id="Layer_2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 27.8 27.8">
  <g id="Layer_1-2" data-name="Layer_1">
    <g>
      <circle cx="13.9" cy="13.9" r="13.4" fill="#1b9c28" stroke="#1b9c28" strokeMiterlimit={10}/>
      <g>
        <path d="M8.54,19.16l-3.37-9.46h1.86l1.68,4.91c.16.49.33,1.06.51,1.7.18.64.38,1.37.59,2.17h-.36c.21-.82.41-1.55.58-2.18.18-.63.34-1.2.49-1.69l1.62-4.91h1.85l-3.31,9.46h-2.15Z" fill="#fff"/>
        <path d="M18.85,19.28c-.87,0-1.64-.2-2.29-.59s-1.17-.95-1.54-1.68-.55-1.58-.55-2.58.19-1.89.57-2.62c.38-.73.89-1.28,1.55-1.67.65-.39,1.39-.58,2.22-.58.52,0,1.01.08,1.46.23.45.15.85.37,1.21.65.35.28.64.61.87,1,.23.39.38.81.45,1.27h-1.73c-.08-.25-.18-.48-.31-.68-.13-.2-.29-.37-.48-.51-.19-.14-.4-.25-.64-.33-.24-.07-.5-.11-.79-.11-.51,0-.96.13-1.36.39-.4.26-.71.63-.94,1.13s-.34,1.1-.34,1.82.11,1.31.34,1.81c.23.5.54.88.95,1.14.4.26.87.39,1.4.39.48,0,.9-.09,1.25-.28.35-.19.63-.45.82-.79.19-.34.29-.75.29-1.21l.39.06h-2.57v-1.33h3.83v1.14c0,.81-.17,1.51-.52,2.1-.35.59-.82,1.04-1.42,1.36-.6.32-1.29.48-2.07.48Z" fill="#fff"/>
      </g>
    </g>
  </g>
</svg>
  );
};
export const VIcon = () => {
    return (
<svg id="Layer_2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 27.8 27.8">
  <g id="Layer_1-2" data-name="Layer_1">
    <g>
      <circle cx="13.9" cy="13.9" r="13.4" fill="#8bc53d" stroke="#8bc53d" strokeMiterlimit={10}/>
      <path d="M12.63,19.16l-3.37-9.46h1.86l1.68,4.91c.16.49.33,1.06.51,1.7.18.64.38,1.37.59,2.17h-.36c.21-.82.41-1.55.58-2.18.18-.63.34-1.2.49-1.69l1.62-4.91h1.85l-3.31,9.46h-2.15Z" fill="#fff"/>
    </g>
  </g>
</svg>
    )
}

export const GFIcon = () => {
    return(
        <svg id="Layer_2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 27.8 27.8">
  <g id="Layer_1-2" data-name="Layer_1">
    <g>
      <circle cx="13.9" cy="13.9" r="13.4" fill="#8b5d39" stroke="#8b5d39" strokeMiterlimit={10}/>
      <g>
        <path d="M10,19.47c-.87,0-1.64-.2-2.29-.59s-1.17-.95-1.54-1.68-.55-1.58-.55-2.58.19-1.89.57-2.62c.38-.73.89-1.28,1.55-1.67.65-.39,1.39-.58,2.22-.58.52,0,1.01.08,1.46.23.45.15.85.37,1.21.65.35.28.64.61.87,1,.23.39.38.81.45,1.27h-1.73c-.08-.25-.18-.48-.31-.68-.13-.2-.29-.37-.48-.51-.19-.14-.4-.25-.64-.33-.24-.07-.5-.11-.79-.11-.51,0-.96.13-1.36.39-.4.26-.71.63-.94,1.13s-.34,1.1-.34,1.82.11,1.31.34,1.81c.23.5.54.88.95,1.14.4.26.87.39,1.4.39.48,0,.9-.09,1.25-.28.35-.19.63-.45.82-.79.19-.34.29-.75.29-1.21l.39.06h-2.57v-1.33h3.83v1.14c0,.81-.17,1.51-.52,2.1-.35.59-.82,1.04-1.42,1.36-.6.32-1.29.48-2.07.48Z" fill="#fff"/>
        <path d="M15.66,19.34v-9.46h6.06v1.43h-4.36v2.84h3.94v1.41h-3.94v3.78h-1.69Z" fill="#fff"/>
      </g>
    </g>
  </g>
</svg>
    )
}
export const FishIcon = () => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      fill="none"
      stroke="#62b1f6"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      className="icon icon-tabler icons-tabler-outline icon-tabler-fish"
    >
      <path stroke="none" d="M0 0h24v24H0z" fill="none" />
      <path d="M16.69 7.44a6.973 6.973 0 0 0 -1.69 4.56c0 1.747 .64 3.345 1.699 4.571" />
      <path d="M2 9.504c7.715 8.647 14.75 10.265 20 2.498c-5.25 -7.761 -12.285 -6.142 -20 2.504" />
      <path d="M18 11v.01" />
      <path d="M11.5 10.5c-.667 1 -.667 2 0 3" />
    </svg>
  );
};

export const EggIcon = () => {
  return (
    <svg
      fill="#000000"
      viewBox="0 0 24 24"
      id="egg"
      data-name="Line Color"
      xmlns="http://www.w3.org/2000/svg"
      className="icon line-color"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        <circle
          id="secondary"
          cx="12"
          cy="14"
          r="3"
          fill="#ffe87c"
          stroke="#ffa500"
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth={1}
        ></circle>
        <path
          id="primary"
          d="M19,15c0,3.31-3.14,6-7,6s-7-2.69-7-6c0-3,1-5.12,2.12-7.5S9.63,3,12,3s3.71,2.05,4.88,4.5S19,12,19,15Z"
          fill="none"
          stroke="#ffa500"
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth={1}
        ></path>
      </g>
    </svg>
  );
};

export const GlutenFreeIcon = () => {
  return (
    <svg
      fill="#8b5d39"
      version="1.1"
      id="Capa_1"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 1125.628 1125.628"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <g>
          {" "}
          <path d="M562.812,0.002C252.476,0.002,0,252.478,0,562.814s252.476,562.812,562.812,562.812 c310.34,0,562.816-252.476,562.816-562.812S873.152,0.002,562.812,0.002z M549.519,701.492c-14,15.033-28.964,28.403-52.992,40.11 c-24.032,11.711-54.35,17.564-91.051,17.564c-46.662,0-85.943-14.504-115.476-43.517c-29.537-29.009-45.481-66.672-45.481-112.986 v-79.694c0-46.139,15.398-83.758,43.801-112.856c28.398-29.099,65.568-43.648,110.311-43.648 c46.487,0,82.395,11.318,107.126,33.948c24.729,22.635,37.507,52.898,38.031,89.77l-0.374,2.081h-72.092 c-1.401-22-7.864-36.816-19.399-48.613c-11.534-11.796-28.402-17.949-50.594-17.949c-23.422,0-41.642,8.909-55.969,26.981 c-14.332,18.073-20.841,41.207-20.841,69.667v80.144c0,28.984,6.77,52.512,21.627,70.585 c14.853,18.072,34.539,27.104,59.705,27.104c17.826,0,32.671-1.827,43.51-5.489c10.834-3.658,16.159-8.188,25.159-13.591v-69.838 h-71v-53h146V701.492z M880.519,430.263h-190v106h163v57h-163v159h-75v-379h265V430.263z"></path>{" "}
        </g>{" "}
      </g>
    </svg>
  );
};

export const CrustaceanIcon = () => {
  return (
    <svg
      fill="#e94444"
      version="1.1"
      id="Layer_1"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        {" "}
        <g>
          {" "}
          <g>
            {" "}
            <path d="M506.236,218.985c-4.383-4.383-10.576-6.441-16.71-5.559c-19.305,2.781-83.919,13.48-104.837,34.401 c-12.383,12.38-19.202,28.843-19.202,46.353c0,3.785,0.337,7.515,0.962,11.172l-42.735-2.107 c9.649-15.346,13.888-29.743,13.098-43.961c-0.738-13.288-5.838-24.556-12.905-34.598l98.94-53.273 c9.563-5.149,13.141-17.075,7.992-26.638c-5.15-9.563-17.075-13.142-26.638-7.992l-62.799,33.812l33.812-62.797 c5.15-9.563,1.571-21.49-7.992-26.638c-9.563-5.147-21.488-1.571-26.638,7.992l-53.267,98.929 c-10.891-7.666-23.194-12.987-37.866-12.987c-13.195,0-26.536,4.277-40.695,13.18l-2.107-42.728 c3.657,0.625,7.388,0.962,11.172,0.962c17.509,0,33.971-6.818,46.351-19.199c20.922-20.922,31.622-85.534,34.402-104.838 c0.884-6.134-1.176-12.327-5.559-16.709c-4.383-4.383-10.576-6.441-16.71-5.559C257,2.981,192.387,13.678,171.467,34.6 c-12.383,12.38-19.202,28.843-19.202,46.353c0,14.76,4.861,28.763,13.8,40.221l4.909,99.531 c-11.029,11.72-21.869,25.098-31.67,39.252l-20.948-20.95c-7.68-7.679-20.133-7.678-27.812-0.001 c-7.68,7.68-7.68,20.131-0.001,27.813l27.826,27.829c-6.908,13.425-12.412,26.898-15.874,39.749L62.73,294.633 c-7.679-7.679-20.131-7.679-27.812,0s-7.68,20.131,0,27.812l53.055,53.056l-73.278,19.14c-6.819,1.78-12.164,7.075-14.009,13.878 c-1.846,6.802,0.09,14.072,5.074,19.055l78.664,78.664c3.738,3.738,8.759,5.761,13.906,5.761c1.716,0,3.447-0.226,5.149-0.686 c6.802-1.846,12.097-7.19,13.878-14.01l19.14-73.278l52.991,52.991c3.84,3.84,8.873,5.761,13.906,5.761 c5.033,0,10.066-1.921,13.905-5.761c7.68-7.679,7.68-20.131,0-27.812l-39.75-39.75c12.85-3.47,26.322-8.982,39.742-15.884 l27.823,27.822c3.84,3.84,8.873,5.76,13.905,5.76s10.066-1.921,13.906-5.761c7.68-7.68,7.679-20.131,0-27.813l-20.931-20.93 c14.217-9.824,27.619-20.657,39.287-31.624l99.544,4.909c11.455,8.938,25.46,13.8,40.221,13.8c17.509,0,33.97-6.818,46.35-19.199 c20.922-20.922,31.622-85.535,34.402-104.841C512.68,229.56,510.619,223.368,506.236,218.985z M87.971,454.165l-30.139-30.139 l40.795-10.656L87.971,454.165z M199.276,62.415c5.322-5.322,28.551-12.809,55.49-18.461c-0.755,3.638-1.592,7.469-2.508,11.374 c-8.564,36.513-15.607,43.877-15.902,44.172c-4.952,4.951-11.536,7.678-18.54,7.678s-13.589-2.727-18.542-7.678 c-4.953-4.953-7.682-11.539-7.682-18.544C191.594,73.952,194.322,67.369,199.276,62.415z M269.837,307.057 c-37.04,37.04-90.957,67.174-120.187,67.174c-4.586,0-8.176-0.852-9.602-2.279c-3.487-3.487-4.464-19.446,7.761-47.42 c12.268-28.07,33.625-58.863,57.134-82.37c18.145-18.145,33.536-27.736,44.508-27.736c9.259,0,18.145,6.983,29.444,18.261 c0.016,0.016,0.031,0.031,0.047,0.049c0.024,0.024,0.049,0.045,0.072,0.069c0.03,0.03,0.06,0.059,0.092,0.089 c10.709,10.706,17.941,19.662,18.436,28.571C298.157,272.551,288.577,288.315,269.837,307.057z M465.483,268.55 c-8.564,36.514-15.607,43.879-15.902,44.174c-4.952,4.951-11.535,7.679-18.539,7.679c-7.005,0-13.589-2.728-18.544-7.682 c-4.953-4.952-7.68-11.536-7.68-18.54s2.728-13.587,7.683-18.542c5.322-5.322,28.551-12.809,55.49-18.462 C467.236,260.816,466.398,264.647,465.483,268.55z"></path>{" "}
          </g>{" "}
        </g>{" "}
      </g>
    </svg>
  );
};

export const VegetarianIcon = (props: Props) => {
  return (
    <div className={`aspect-square w-[2rem]  flex  rounded-full font-semibold justify-center items-center bg-[#8bc53d] text-white  text-[clamp(6px,85%,16px)] p-[1px] w-[${props.width}] h-[${props.width}]  `}>
      V
    </div>
  );
};

export const HalalIcon = () => {
  return (
    <svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
      <title />
      <g>
        <path
          fill="#1a4f2d"
          d="M52.113,24.323a.926.926,0,0,1-.273-.66V15.094a2.937,2.937,0,0,0-2.934-2.934H40.337a.932.932,0,0,1-.661-.273l-5.6-5.6a2.939,2.939,0,0,0-4.149,0l-5.6,5.6a.926.926,0,0,1-.66.273H15.094a2.937,2.937,0,0,0-2.934,2.934v8.569a.941.941,0,0,1-.273.66l-5.6,5.6a2.933,2.933,0,0,0,0,4.148l5.6,5.6a.938.938,0,0,1,.273.66v8.569a2.937,2.937,0,0,0,2.934,2.934h8.569a.926.926,0,0,1,.66.273l5.6,5.6a2.937,2.937,0,0,0,4.149,0l5.6-5.6a.932.932,0,0,1,.661-.273h8.569a2.937,2.937,0,0,0,2.934-2.934V40.337a.943.943,0,0,1,.273-.66h0l5.6-5.6a2.932,2.932,0,0,0,0-4.149ZM56.3,32.66l-5.6,5.6a2.952,2.952,0,0,0-.859,2.074v8.569a.935.935,0,0,1-.934.934H40.337a2.917,2.917,0,0,0-2.075.859l-5.6,5.6a.933.933,0,0,1-1.32,0l-5.6-5.6a2.913,2.913,0,0,0-2.074-.859H15.094a.935.935,0,0,1-.934-.934V40.337a2.953,2.953,0,0,0-.859-2.074l-5.6-5.6a.934.934,0,0,1,0-1.32l5.6-5.6a2.916,2.916,0,0,0,.859-2.074V15.094a.935.935,0,0,1,.934-.934h8.569a2.913,2.913,0,0,0,2.074-.859l5.6-5.6a.934.934,0,0,1,1.32,0l5.6,5.6a2.917,2.917,0,0,0,2.075.859h8.569a.935.935,0,0,1,.934.934v8.569a2.913,2.913,0,0,0,.859,2.074l5.6,5.6a.932.932,0,0,1,0,1.32Z"
        />
        <path
          fill="#1a4f2d"
          d="M47.021,26.151a.425.425,0,0,1-.124-.3V19.53A2.429,2.429,0,0,0,44.47,17.1H38.15a.426.426,0,0,1-.3-.125l-4.133-4.133a2.429,2.429,0,0,0-3.431,0L26.1,17.023a.424.424,0,0,1-.244.081H19.53A2.428,2.428,0,0,0,17.1,19.53v6.321a.5.5,0,0,1-.125.3l-4.133,4.132a2.425,2.425,0,0,0,0,3.431l4.133,4.133a.426.426,0,0,1,.125.3v6.32A2.429,2.429,0,0,0,19.53,46.9h6.321a.427.427,0,0,1,.3.125l4.132,4.131a2.427,2.427,0,0,0,3.431,0l4.133-4.131a.422.422,0,0,1,.3-.125h6.32A2.444,2.444,0,0,0,46.9,44.47V38.15a.423.423,0,0,1,.124-.3l4.132-4.132a2.426,2.426,0,0,0,0-3.432Zm2.718,6.15-4.132,4.133a2.41,2.41,0,0,0-.71,1.716v6.32a.421.421,0,0,1-.161.325.409.409,0,0,1-.266.1H38.15a2.408,2.408,0,0,0-1.716.711L32.3,49.739a.428.428,0,0,1-.6,0l-4.132-4.131a2.406,2.406,0,0,0-1.715-.711H19.53a.427.427,0,0,1-.426-.427V38.15a2.412,2.412,0,0,0-.711-1.716L14.26,32.3a.426.426,0,0,1,0-.6l4.133-4.132a2.154,2.154,0,0,0,.18-.212,2.434,2.434,0,0,0,.531-1.5V19.53a.426.426,0,0,1,.426-.426h6.321a2.408,2.408,0,0,0,1.4-.447,2.33,2.33,0,0,0,.319-.264L31.7,14.26a.427.427,0,0,1,.6,0l4.133,4.133a2.412,2.412,0,0,0,1.716.711h6.32a.427.427,0,0,1,.427.426v6.321a2.44,2.44,0,0,0,.71,1.715L49.739,31.7a.426.426,0,0,1,0,.6Z"
        />
        <path
          fill="#1a4f2d"
          d="M27.69,26.517a1,1,0,0,0-1.992.174c.237,2.744-.054,6.953-1.445,7.716a1.911,1.911,0,0,1-2,.114c-.393-.325-.449-1.037-.154-1.955a1,1,0,1,0-1.9-.612c-.794,2.469.285,3.7.785,4.109a3.171,3.171,0,0,0,2.043.69,4.578,4.578,0,0,0,2.189-.593C28.228,34.506,27.805,27.837,27.69,26.517Z"
        />
        <path
          fill="#1a4f2d"
          d="M43.781,31.88a5.28,5.28,0,0,1-1.527-.711c-.543-.414-2.451-1.733-4.214-1.349a2.745,2.745,0,0,0-1.861,1.46,1,1,0,0,0,1.766.939.742.742,0,0,1,.523-.445,4.03,4.03,0,0,1,2.573.985,5.279,5.279,0,0,0,.835.51,11.259,11.259,0,0,1-4.636,1.555,2.344,2.344,0,0,1-1.73-.309c-.383-.39-.283-1.408-.2-2.226.02-.2.039-.406.053-.6a30.412,30.412,0,0,1,1.2-4.99,1,1,0,0,0-1.9-.618c-.033.1-.582,1.8-.957,3.465a10.731,10.731,0,0,0-3.805-3.037,1,1,0,0,0-.83,1.819c.04.018,3.94,1.872,3.623,4.409a3.264,3.264,0,0,1-1.766,2.273c-.234.087-.942.349-2.014-.619a1,1,0,1,0-1.341,1.483A4.2,4.2,0,0,0,30.41,37.1a3.466,3.466,0,0,0,1.2-.216A4.693,4.693,0,0,0,33.642,35.3a2.657,2.657,0,0,0,.442.617,3.574,3.574,0,0,0,2.683.928c.2,0,.412-.009.632-.026a13.845,13.845,0,0,0,6.756-2.646,1.3,1.3,0,0,0-.374-2.292Z"
        />
      </g>
    </svg>
  );
};

export const HalalIcon2 = () => {
  return (
    <svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
      <title />
      <g>
        <path
          d="M50.447,30.991l-2.071-2.07L46.859,27.4l-.545-.545a1.426,1.426,0,0,1-.417-1.008V19.53A1.427,1.427,0,0,0,44.47,18.1H38.149a1.426,1.426,0,0,1-1.008-.417l-4.132-4.133a1.427,1.427,0,0,0-2.018,0l-3.423,3.423-.709.71a1.452,1.452,0,0,1-.188.154,1.427,1.427,0,0,1-.82.263H19.53A1.427,1.427,0,0,0,18.1,19.53v6.321a1.42,1.42,0,0,1-.313.881,1.4,1.4,0,0,1-.1.127l-3.28,3.279-.853.853a1.427,1.427,0,0,0,0,2.018l4.133,4.132a1.426,1.426,0,0,1,.417,1.008V44.47A1.427,1.427,0,0,0,19.53,45.9h6.321a1.426,1.426,0,0,1,1.008.417l4.132,4.133a1.427,1.427,0,0,0,2.018,0l1.68-1.681,2.452-2.452a1.426,1.426,0,0,1,1.008-.417H44.47A1.445,1.445,0,0,0,45.9,44.47V38.149a1.426,1.426,0,0,1,.417-1.008l4.133-4.132A1.427,1.427,0,0,0,50.447,30.991Z"
          fill="#1a4f2d"
        />
        <g>
          <path
            d="M30.991,50.447l-4.132-4.133a1.426,1.426,0,0,0-1.008-.417H19.53A1.427,1.427,0,0,1,18.1,44.47V38.149a1.426,1.426,0,0,0-.417-1.008l-4.133-4.132a1.427,1.427,0,0,1,0-2.018l.853-.853,3.28-3.279a1.4,1.4,0,0,0,.1-.127,1.42,1.42,0,0,0,.313-.881V19.53A1.427,1.427,0,0,1,19.53,18.1h6.321a1.427,1.427,0,0,0,.82-.263,1.452,1.452,0,0,0,.188-.154l.709-.71,3.423-3.423A1.425,1.425,0,0,1,32,13.136V6.425a1.925,1.925,0,0,0-1.367.567l-5.6,5.6a2.02,2.02,0,0,1-1.367.566H15.094a1.934,1.934,0,0,0-1.934,1.934v8.569a1.927,1.927,0,0,1-.425,1.2,1.791,1.791,0,0,1-.141.172l-5.6,5.6a1.931,1.931,0,0,0,0,2.734l5.6,5.6a1.934,1.934,0,0,1,.566,1.367v8.569a1.934,1.934,0,0,0,1.934,1.934h8.569a1.934,1.934,0,0,1,1.367.566l5.6,5.6A1.925,1.925,0,0,0,32,57.575V50.864A1.425,1.425,0,0,1,30.991,50.447Z"
            fill="#2aab48"
          />
          <path
            d="M38.97,51.406a1.934,1.934,0,0,1,1.367-.566h8.569a1.957,1.957,0,0,0,1.934-1.934V40.337a1.934,1.934,0,0,1,.566-1.367l5.6-5.6a1.931,1.931,0,0,0,0-2.734L54.2,27.826l-2.8-2.8a1.934,1.934,0,0,1-.566-1.367V15.094a1.934,1.934,0,0,0-1.934-1.934H40.337a1.934,1.934,0,0,1-1.367-.566l-5.6-5.6A1.925,1.925,0,0,0,32,6.425v6.711a1.425,1.425,0,0,1,1.009.417l4.132,4.133a1.426,1.426,0,0,0,1.008.417H44.47A1.427,1.427,0,0,1,45.9,19.53v6.321a1.426,1.426,0,0,0,.417,1.008l.545.545,1.517,1.517,2.071,2.07a1.427,1.427,0,0,1,0,2.018l-4.133,4.132a1.426,1.426,0,0,0-.417,1.008V44.47A1.445,1.445,0,0,1,44.47,45.9H38.149a1.426,1.426,0,0,0-1.008.417l-2.452,2.452-1.68,1.681A1.425,1.425,0,0,1,32,50.864v6.711a1.925,1.925,0,0,0,1.367-.567Z"
            fill="#5fc076"
          />
          <path
            d="M27.568,16.976l-.709.71a1.452,1.452,0,0,1-.188.154,1.427,1.427,0,0,1-.82.263H19.53A1.427,1.427,0,0,0,18.1,19.53v6.321a1.42,1.42,0,0,1-.313.881,1.4,1.4,0,0,1-.1.127l-3.28,3.279-.853.853a1.427,1.427,0,0,0,0,2.018l4.133,4.132a1.426,1.426,0,0,1,.417,1.008V44.47A1.427,1.427,0,0,0,19.53,45.9h6.321a1.426,1.426,0,0,1,1.008.417l4.132,4.133A1.425,1.425,0,0,0,32,50.864V13.136a1.425,1.425,0,0,0-1.009.417Z"
            fill="#1a4f2d"
          />
        </g>
        <path
          d="M51.406,38.97l5.6-5.6a1.931,1.931,0,0,0,0-2.734l-5.6-5.6a1.934,1.934,0,0,1-.566-1.367V15.094a1.934,1.934,0,0,0-1.934-1.934H40.337a1.934,1.934,0,0,1-1.367-.566l-5.6-5.6a1.931,1.931,0,0,0-2.734,0l-5.6,5.6a1.934,1.934,0,0,1-1.367.566H15.094a1.934,1.934,0,0,0-1.934,1.934v8.569a1.934,1.934,0,0,1-.566,1.367l-5.6,5.6a1.931,1.931,0,0,0,0,2.734l5.6,5.6a1.934,1.934,0,0,1,.566,1.367v8.569a1.934,1.934,0,0,0,1.934,1.934h8.569a1.934,1.934,0,0,1,1.367.566l5.6,5.6a1.931,1.931,0,0,0,2.734,0l5.6-5.6a1.934,1.934,0,0,1,1.367-.566h8.569a1.934,1.934,0,0,0,1.934-1.934V40.337A1.934,1.934,0,0,1,51.406,38.97Z"
          fill="none"
          stroke="#2aab48"
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth="2"
        />
        <path
          d="M50.447,30.991l-2.071-2.07L46.859,27.4l-.545-.545a1.426,1.426,0,0,1-.417-1.008V19.53A1.427,1.427,0,0,0,44.47,18.1H38.149a1.426,1.426,0,0,1-1.008-.417l-4.132-4.133a1.427,1.427,0,0,0-2.018,0l-3.423,3.423-.709.71a1.452,1.452,0,0,1-.188.154,1.427,1.427,0,0,1-.82.263H19.53A1.427,1.427,0,0,0,18.1,19.53v6.321a1.42,1.42,0,0,1-.313.881,1.4,1.4,0,0,1-.1.127l-3.28,3.279-.853.853a1.427,1.427,0,0,0,0,2.018l4.133,4.132a1.426,1.426,0,0,1,.417,1.008V44.47A1.427,1.427,0,0,0,19.53,45.9h6.321a1.426,1.426,0,0,1,1.008.417l4.132,4.133a1.427,1.427,0,0,0,2.018,0l1.68-1.681,2.452-2.452a1.426,1.426,0,0,1,1.008-.417H44.47A1.445,1.445,0,0,0,45.9,44.47V38.149a1.426,1.426,0,0,1,.417-1.008l4.133-4.132A1.427,1.427,0,0,0,50.447,30.991Z"
          fill="none"
          stroke="#231f20"
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth="2"
        />
        <g>
          <path
            d="M26.694,26.6s.633,7.256-1.96,8.68c-2.16,1.186-4.592.112-3.584-3.024"
            fill="none"
            stroke="#ffffff"
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth="2"
          />
          <path
            d="M35.6,26.393a31.111,31.111,0,0,0-1.243,5.228c-.128,1.8-.814,4.5,2.957,4.2a12.992,12.992,0,0,0,6.25-2.459.3.3,0,0,0-.089-.528,6.384,6.384,0,0,1-1.832-.87c-.9-.686-3.514-2.228-4.586-.214"
            fill="none"
            stroke="#ffffff"
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth="2"
          />
          <path
            d="M29.477,27.422s4.617,2.105,4.2,5.442c-.2,1.618-2.568,4.87-5.443,2.271"
            fill="none"
            stroke="#ffffff"
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth="2"
          />
        </g>
      </g>
    </svg>
  );
};

export const DairyIcon = () => {
  return (
    <svg
      fill="#3ba9fd"
      version="1.1"
      id="Layer_1"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
    >
      <g>
        <g>
          <path
            d="M379.939,83.701v-53.75C379.939,13.435,365.257,0,347.211,0H164.789c-18.047,0-32.728,13.435-32.728,29.951v53.75
			L39.795,187.403V512h432.41V187.403L379.939,83.701z M162.478,30.967c0.422-0.246,1.192-0.55,2.311-0.55h182.422
			c1.119,0,1.889,0.303,2.311,0.55v43.312H162.478V30.967z M224.326,481.583H70.212V198.976l77.057-86.608l77.057,86.608V481.583z
			 M181.158,104.696h176.747l65.203,73.285H246.359L181.158,104.696z M441.788,481.583H254.743V208.398h187.045V481.583z"
          />
        </g>
      </g>
    </svg>
  );
};
