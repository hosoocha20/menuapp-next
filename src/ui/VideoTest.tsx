...
const VideoTest = () => {
  ...
  return (
    <div className="h-[200vh] flex justify-center items-center">
      <div className="h-[900px] relative">
        <img src={"/phonemenudesign111.svg"} className="h-[900px]" />
        <div
          className={`absolute bg-black  w-[92%] h-[96%] top-[18px] left-[20px] rounded-[1rem] ${
            show ? "opacity-45" : "opacity-0"
          } transition-opacity duration-500`}
        ></div>
        <div
          className={`videotest absolute border px-5 py-4 w-[82%] h-[85%] top-[93px] left-[42px] rounded-[1rem]  overflow-y-auto bg-white ${
            show ? "opacity-1 z-[10]" : "opacity-0"
          } transition-opacity duration-500`}
        >
          <div className="w-full flex justify-end">
          ...
          <p className="text-center font-medium mt-[-20px]">Prosciutto Pizza</p>
          <img src="/prosciuttopizza.jpeg" className="w-[120px] m-auto mt-3" />
          <hr className="mt-3"></hr>
          <p className="mt-5 text-[15px]">
            A classic Italian delight featuring a slow-simmered tomato sauce,
            melted mozzarella cheese, savory prosciutto slices, fresh basil
            leaves, and a drizzle of olive oil, served on a thin, crispy crust.
          </p>

          <div className="absolute bottom-4 px-5 left-0 w-full">
            <hr className="mt-5"></hr>
            <div className="grid grid-cols-2 mt-5 *:text-[15px]">
              <p className="font-semibold">Selected:</p>
              <p className="font-semibold">Amount</p>
              ...
              <p>$29.0</p>
              <p className="font-bold text-[16px] mt-2">Total</p>
              <p className="font-bold text-[16px] mt-2">$29.0</p>
            </div>
            <button className="border py-2 px-3 rounded-[0.5rem] float-right flex items-center bg-[#14452f] text-white mt-5">
              <IoCheckmark />
              <p className="text-[15px]">Add to Cart</p>
                    ...
                    <div
            className=" absolute  w-[150px] h-[150px] right-[30px] top-[380px] cursor-pointer"
            onClick={() => setShow(true)}
                    ></div>
          </div>
      </div>
